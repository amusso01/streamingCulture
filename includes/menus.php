  ...
  ** ***** ----------------------------------------------- ***** */
  ...
  register_nav_menus(
    array(
      ...
      'secondary_menu' => __( 'Secondary Menu', 'bymattlee' ),
  ...
  // Custom walker for secondary menu that removes all unnecessary classes
  class Secondary_Nav_Walker extends Walker_Nav_Menu {

    // Rename sub menu class name
    function start_lvl( &$output, $depth = 0, $args = array() ) {
      $indent = str_repeat("\t", $depth);
      $output .= "\n$indent<ul class=\"o-nav-secondary-subMenu\">";
    }
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
      ...
      // Define new array of classes with new naming structure
      $list_classes = array( 'o-nav-secondary-menuListItem', 'js-nav-menuListItem' );
      ...
      if(!get_field('link_attivo', $item->ID)){
        $list_classes[] = 's-disabled';
      }

      // If class array contains 'menu-item-has-children', rename to 'has_submenu' and add to the list tag class array
      if ( in_array( 'menu-item-has-children', $classes ) ) $list_classes[] = 'o-nav-secondary-menuListItem--hasSubmenu';
      ...
      $attributes .= ! empty( $item->xfn )        ? ' rel="'    . esc_attr( $item->xfn        ) .'"' : '';
      $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
      $attributes .= ' class="o-nav-secondary-menuItem js-nav-menuItem"';
...
function sp_wp_nav_menu_objects( $items, $args ) {
  ...
  foreach( $items as &$item ) {
    // vars
    $icon = get_field('link_attivo', $item);
    // append icon
    if( $icon ) {
      $item->classes[] = 's-disabled';
    }
  }
  // return
  return $items;
